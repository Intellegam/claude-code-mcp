...
/** Latest real request input, including cached input; not post-turn fullness. */
export function normalizeAssistantUsage(message) {
  ...
  const contextTokens = values.reduce((sum, value) => sum + value, 0);
  return contextTokens > 0 && Number.isFinite(contextTokens)
    ? { contextTokens }
    : null;
}
...
export class ClaudeRunner {
   ...
   *   onUsage(usage)         — latest real request input-token count
   *   onCompact()            — an observed compact boundary this turn
      ...
      for await (const message of this.#q) {
        if (message?.type === "system" && message.subtype === "init") {
          if (!this.#initialized) {
            ...
            if (message.model) events.onModel?.(message.model);
...
export function createRunnerFactory({ query, getSessionInfo }) {
  return function createRunner({
    ...
    return new ClaudeRunner({
      query: scopedQuery,
      options: buildQueryOptions({ cwd, writable, model, resume }),
    });
  };
}
