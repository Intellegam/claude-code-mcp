...
export class ClaudeRunner {
   ...
   *   onInit(sessionId, model) — `system/init` seen (once per turn); `model` is
   *                            the model the CLI resolved for the turn
   *   onModel(model)         — an assistant message's serving model; refines the
   *                            init resolution when the CLI falls back mid-turn
      ...
      for await (const message of this.#q) {
        if (message?.type === "system" && message.subtype === "init") {
          if (!this.#initialized) {
            ...
            events.onInit?.(message.session_id ?? null, message.model ?? null);
 ...
 */
export function createRunnerFactory({ query }) {
