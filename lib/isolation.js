 ...
 *
 * `Task` is the delegation tool. The rest are the same class of capability in
 * SDK 0.3.220's tool surface, which is wider than `Task` alone.
 */
 ...
 *
 * `Monitor` is here because it executes shell commands (its own guidance tells
 * the model to run `until <check>; do sleep 2; done` in it) — without it,
 * blocking `Bash` would not actually make the session read-only.
 */
...
export const WRITABLE_DISALLOWED_TOOLS = [...ALWAYS_DISALLOWED_TOOLS];

export const MCP_DENY_MESSAGE =
  "MCP tools are not available in this consultation";

/**
 * The runtime authorization seam: a `canUseTool` callback consulted per tool
 * call, in both permission modes.
 *
 * Today it does one thing — default-deny every `mcp__*` tool.
 * `strictMcpConfig` should already keep MCP servers from loading at all, but
 * this is the belt-and-braces boundary: `disallowedTools` is a static list and
 * cannot cover dynamically named `mcp__<server>__<tool>` names.
 *
 * It is a function of the mode so that a later deployment-level allowlist
 * (per-server read/write tool patterns, e.g. an observability server) has an
 * obvious place to plug in.
 */
export function buildToolPolicy({ writable = false } = {}) {
  return async function canUseTool(toolName, input) {
    if (typeof toolName === "string" && toolName.startsWith("mcp__")) {
      return { behavior: "deny", message: MCP_DENY_MESSAGE };
    }
    // Everything else is governed statically by disallowedTools /
    // permissionMode; nothing that reaches this point needs a second opinion.
...
export function buildQueryOptions({
  ...
  const options = {
    ...
    canUseTool: buildToolPolicy({ writable }),
  };
