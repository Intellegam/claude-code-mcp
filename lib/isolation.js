 ...
 *
 * Nothing here overrides a rule the operator wrote: the deny hook is terminal,
 ...
 *
 * *Verified (0.3.220):* an out-of-tree `Read`, `Glob` or `Grep` raises a
 * permission request whose `decisionReason` is exactly "Path is outside
 * allowed working directories", while a request forced by an operator
 * `permissions.ask` rule arrives with NO `decisionReason` (and, despite the
 * SDK types, no `matchedAskRule` either). Matching the reason is therefore the
 * only way to approve the gate without also swallowing requests the operator
 * reserved for a human. The string is not contractual — but the SDK is exactly
 * pinned, upgrades are release-gated, and the tier-2 tests fail closed (reads
 * lose access) if it ever changes.
 */
