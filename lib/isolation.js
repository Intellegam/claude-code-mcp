 ...
 * or `claude`, optionally decorated with `-code`/`-agent`/`-mcp` — and the
 * trailing `__` is what makes that a boundary rather than a prefix: an
 * unrelated `mcp__codexdb__*` or `mcp__claude-agent-inbox__*` must not be
 * caught. Every other MCP server is allowed on purpose — they are part of the
 * environment the operator already works in.
 */
export const AGENT_BRIDGE_PATTERN =
  /^mcp__(codex|claude)(?:[-_](?:code|agent|mcp))*__/i;
