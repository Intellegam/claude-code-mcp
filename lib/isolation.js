...
/**
 * Built-in tools the read-only callback approves outside the working
 * directory.
 *
 * Read visibility is deliberately unrestricted — cross-repo consultations are
 * the point — but the CLI only auto-allows file reads inside the session `cwd`;
 * an out-of-tree `Read`/`Glob`/`Grep` raises a permission request instead.
 * Approving them here honors the visibility contract while the operator's own
 * `permissions.deny` rules (evaluated before this callback) keep the last word.
 */
export const READ_ONLY_AUTO_APPROVED_TOOLS = new Set(["Read", "Glob", "Grep"]);

/**
 * The `decisionReason` the CLI attaches to the out-of-tree gate.
 *
 * *Verified (0.3.220):* `Read`, `Glob` and `Grep` all raise it with exactly
 * "Path is outside allowed working directories", while a request forced by an
 * operator `permissions.ask` rule arrives with NO `decisionReason` (and,
 * despite the SDK types, no `matchedAskRule` either). Matching the reason is
 * therefore the only way to approve the gate without also swallowing requests
 * the operator reserved for a human. The string is not contractual — but the
 * SDK is exactly pinned, upgrades are release-gated, and the tier-2 tests
 * fail closed (reads lose access) if it ever changes.
 */
 ...
 * human, and headless auto-approval would erase exactly that reservation. An
 * ask-forced request carries no `decisionReason`, so it misses the gate match
 * and lands in the generic deny; the explicit `matchedAskRule` branch exists
 * for the day the CLI starts sending the field it already declares.
 *
...
export function buildReadOnlyPermissionCallback() {
  return async (toolName, input, context) => {
    ...
    // Forward-compatibility only: the pinned CLI never sets `matchedAskRule`
    // (ask-forced requests are recognizable solely by carrying no
    // `decisionReason`, so they fall through to the generic deny below).
