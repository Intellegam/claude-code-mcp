...
export const CACHE_LIKELY_COLD_AFTER_MS = 60 * 60 * 1000;
...
export function createEngine({
  ...
  function snapshotTurn(turn) {
    ...
    const configuredWindow =
      turn.modelContextWindow != null && turn.autoCompactWindow != null
        ? Math.min(turn.modelContextWindow, turn.autoCompactWindow)
        : turn.modelContextWindow;
    const contextWindow =
      turn.reportedContextWindow ??
      configuredWindow ??
      null;
    const idleMs = turn.finishedAt
      ? Math.max(0, Date.now() - turn.finishedAt)
      : 0;
    return {
      ...
      autoCompactWindow: turn.autoCompactWindow,
      autoCompactThreshold: turn.autoCompactThreshold,
      isAutoCompactEnabled: turn.isAutoCompactEnabled,
      contextPercent:
        turn.reportedContextPercent ??
        (turn.contextTokens != null && contextWindow
          ? Number(((turn.contextTokens / contextWindow) * 100).toFixed(1))
          : null),
      compactionCount: session?.compactionCount ?? turn.compactionCount,
      turnCompactionCount: turn.compactionCount,
      lastCompaction: session?.lastCompaction ?? turn.lastCompaction,
      lastRequestAt: new Date(turn.createdAt).toISOString(),
      idleSeconds: Math.floor(idleMs / 1000),
      cacheLikelyCold:
        turn.finishedAt != null && idleMs >= CACHE_LIKELY_COLD_AFTER_MS,
      status: turn.status,
  ...
  function startTurn(
    turn,
    { prompt, writable = false, resume = null, verifyResumeCwd = false },
  ) {
    ...
    try {
      runner.start(prompt, {
        ...
        onUsage: (usage) => {
          if (isTerminal(turn.status)) return;
          turn.contextTokens = usage.contextTokens;
          turn.peakContextTokens = Math.max(
            turn.peakContextTokens ?? 0,
            usage.contextTokens,
          );
        },
        onCompact: (boundary) => {
          ...
          if (boundary.postTokens != null) {
            turn.contextTokens = boundary.postTokens;
          }
          turn.compactionCount += 1;
          turn.lastCompaction = boundary;
          const session = turn.sessionId ? sessions.get(turn.sessionId) : null;
          if (session) {
            session.compactionCount += 1;
            session.lastCompaction = boundary;
          }
        },
        onContextUsage: (usage) => {
          ...
          turn.reportedContextWindow = usage.contextWindow;
          turn.reportedContextPercent = usage.contextPercent;
          turn.modelContextWindow =
            usage.modelContextWindow ?? turn.modelContextWindow;
          turn.autoCompactThreshold =
            usage.autoCompactThreshold ?? turn.autoCompactThreshold;
          turn.isAutoCompactEnabled =
            usage.isAutoCompactEnabled ?? turn.isAutoCompactEnabled;
        },
  ...
  function handleResult(turn, result) {
    if (isTerminal(turn.status)) return;
    turn.modelContextWindow =
      result.modelContextWindow ?? turn.modelContextWindow;
