...
export function createEngine({
  ...
  function snapshotTurn(turn) {
    const session = turn.sessionId ? sessions.get(turn.sessionId) : null;
    const contextWindow = turn.reportedContextWindow ?? null;
    return {
  ...
  function requestTurnCancel(turn, reason = "user") {
    ...
    // The model has finished; optional telemetry must not turn success into a
    // cancellation or timeout. Publish the saved answer and skip the read.
    if (turn.pendingResult) {
      handleResult(turn, turn.pendingResult);
      return turn;
    }
  ...
  function startTurn(
    ...
    try {
      runner.start(prompt, {
        ...
        onResultObserved: (result) => {
          if (!result.isError && !isTerminal(turn.status)) {
